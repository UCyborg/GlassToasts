...
// Balloon notifications are shown by invoking the Shell_NotifyIcon() API.
...
// by Shell_NotifyIcon() calls and thus not show balloons (we
...
HANDLE hMonitorEvent;		// handle to an event that is signalled when the user presses Stop
HANDLE hMonitorThread;		// handle to a thread that monitors the thread handling the taskbar
...
LRESULT CALLBACK MsgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	...
	if (hWnd == tray && WM_COPYDATA == uMsg && !inSendMessage)
	{
		if (owner)
		{
			...
			if (res == 2)
			{
				// some flags remain to be handled by tray window's code
				COPYDATASTRUCT cds;
				TRAYNOTIFYDATA tnd;
...
__declspec(dllexport) BOOL APIENTRY StartHook(HWND hwnd)
{
	...
	if (hook)
	{
		hMonitorEvent = CreateEvent(NULL, FALSE, FALSE, _T("AveTrayMonitorEvent"));
		if (!hMonitorEvent)
		{
			StopHook();
			return FALSE;
