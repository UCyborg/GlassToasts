...
BOOL HasDWM()
{
	...
	if (bDwmLib)
	{
		__try
		{
			// dwmapi.dll containing the following function is delay-loaded
			// since neither exists on Windows 2000/XP, exception will be thrown...
			DwmIsCompositionEnabled(&bEnabled);
		}
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
			// ...so we don't bother trying to invoke it again
			bDwmLib = FALSE;
		}
	}
	return bEnabled;
...
LRESULT CMainDlg::OnCopyData(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
{
	...
	if (data && data->dwSignature == 0x34753423)
	{
		if (data->dwMessage <= NIM_MODIFY)
		{
			if (data->nid.uFlags & NIF_INFO)
			{
				...
				if (test)
				{
					// flags related to balloon notifications handled, but some remained unhandled
					// signal that the message should be forwarded to tray window without balloon notifications' flags
					return 2;
				}
				// we're done, no need to forward anything to tray window
				return 1;
			}
		}
	...
	// forward the message as-it-is to tray window
	return 0;
}
...
{
	// when Explorer crashes or anything happens with the thread handling the taskbar, activate the timer
	// timer event handler will periodically attempt to re-activate the hook
	// user may implicitly stop the timer by pressing Start
...
HBITMAP paintBalloon(NOTIFYICONDATA32* data, SIZE* size, Rect* closeRc, Rect* optionsRc, int states, HWND hwnd)
{
	...
	if (data->dwBalloonIcon)
	{
		...
		if (GetOffsetValue(_T("icon"), _T("usereflection"), 1)) // useReflection
		{
			...
			HDC iconDC = CreateCompatibleDC(NULL);
			BITMAPINFO dib = { 0 };
			dib.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
